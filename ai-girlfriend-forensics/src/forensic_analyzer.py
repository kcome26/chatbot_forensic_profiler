...
==============================================="""
            ...
            if 'text' in df.columns:
                for text in df['text'].dropna():
                    ...
                    if any(phrase in text_lower for phrase in location_phrases):
                        personal_info['location_data'].append(f"Location: {text_str[:150]}")
                    ...
                    if any(phrase in text_lower for phrase in personal_phrases):
                        personal_info['personal_details'].append(f"Personal: {text_str[:150]}")
                    ...
                    if any(phrase in text_lower for phrase in contact_phrases):
                        personal_info['contact_information'].append(f"Contact: {text_str[:150]}")
                    ...
                    if any(phrase in text_lower for phrase in work_phrases):
                        personal_info['work_education'].append(f"Work/Education: {text_str[:150]}")
                    ...
                    if any(phrase in text_lower for phrase in lifestyle_phrases):
                        personal_info['lifestyle_information'].append(f"Interest: {text_str[:150]}")
                    ...
                    if any(phrase in text_lower for phrase in relationship_phrases):
                        personal_info['relationship_details'].append(f"Relationship: {text_str[:150]}")
                    ...
                    if any(phrase in text_lower for phrase in financial_phrases):
                        personal_info['financial_information'].append(f"Financial: {text_str[:150]}")
                    ...
                    if any(phrase in text_lower for phrase in family_phrases):
                        personal_info['family_information'].append(f"Family: {text_str[:150]}")
                    ...
                    if any(phrase in text_lower for phrase in health_phrases):
                        personal_info['health_information'].append(f"Health: {text_str[:150]}")
                    ...
                    if any(phrase in text_lower for phrase in social_phrases):
                        personal_info['social_media'].append(f"Social Media: {text_str[:150]}")
                    ...
                    if any(phrase in text_lower for phrase in schedule_phrases):
                        personal_info['schedule_routine'].append(f"Routine: {text_str[:150]}")
                    ...
                    if any(phrase in text_lower for phrase in struggle_phrases):
                        personal_info['personal_struggles'].append(f"Struggle: {text_str[:150]}")
            ...
            if 'text' in df.columns:
                for text in df['text'].dropna():
                    ...
                    if any(phrase in text_lower for phrase in disclosure_phrases):
                        personal_info['personal_details'].append(f"Self-disclosure: {text_str[:150]}")
                    ...
                    if any(phrase in text_lower for phrase in location_sharing):
                        personal_info['location_data'].append(f"Location sharing: {text_str[:150]}")
                    ...
                    if any(phrase in text_lower for phrase in emotional_phrases):
                        personal_info['health_information'].append(f"Emotional state: {text_str[:150]}")
            ...
            for col in df.columns:
                for value in df[col].dropna():
                    ...
                    if len(value_str) > 1 and value_str not in ['1', '0', 'true', 'false']:
                        personal_info['personal_details'].append(f"Profile data ({col}): {value_str[:150]}")
    ...
    def _extract_key_messages(self) -> List[Dict[str, Any]]:
        ...
        # Extract high sentiment messages from analysis
        for table_data in content_analysis.values():
            for column_data in table_data.values():
                if isinstance(column_data, dict) and 'sentiment_analysis' in column_data:
                    sentiment = column_data['sentiment_analysis']
                    extreme_sentiments = sentiment.get('extreme_sentiments', {})
                    
                    # Add very positive messages
                    if extreme_sentiments.get('most_positive', 0) > 0.8:
                        key_messages.append({
                            'type': 'HIGHLY_POSITIVE',
                            'content': 'High positive sentiment detected in conversation',
                            'sentiment_score': extreme_sentiments.get('most_positive', 0),
                            'priority': 5
                        })
                    
                    # Add very negative messages
                    if extreme_sentiments.get('most_negative', 0) < -0.8:
                        key_messages.append({
                            'type': 'HIGHLY_NEGATIVE',
                            'content': 'High negative sentiment detected in conversation',
                            'sentiment_score': extreme_sentiments.get('most_negative', 0),
                            'priority': 3
                        })
